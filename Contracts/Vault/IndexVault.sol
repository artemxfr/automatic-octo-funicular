...
contract IndexVault is IERC20, Ownable {
    ...
    uint256 rebalanceCycle = 2592000; // 30 days
    ...
    constructor(
        ...
        nextRebalance = block.timestamp + rebalanceCycle;
    }
    ...
    function deposit(uint256 amount) external {
        ...
        if (autoRebalance && nextRebalance <= block.timestamp) {
            _autoRebalance();
        }
    ...
    function withdraw(uint256 amount) external {
        ...
        if (managerFee > 0) {
            managerFeeAmount = totalReceived * managerFee / denominator;
            transferAmount = totalReceived - managerFeeAmount;
            BUSD.safeTransfer(_msgSender(), transferAmount);
            BUSD.safeTransfer(managerAddress, managerFeeAmount);
        } else {
            BUSD.safeTransfer(_msgSender(), totalReceived);
        }
    ...
    function getVaultInfo() external view returns (address[] memory, uint256[] memory, uint256[] memory, uint256[] memory, uint256, uint256, uint256) {
        ...
        return (_assets, _allocations, _amountInBUSD, _tokenAmount, _TVL, _totalUsers, lastRebalance);
    }
    ...
    function setAutoRebalance(bool _value) external onlyOwner {
        autoRebalance = _value;
    }

    function changeRebalanceCycle(uint256 _newCycle) external onlyOwner {
